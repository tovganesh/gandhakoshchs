...
def transformData(contactsData):
    ...
    for contact in contactsData:
        ...
        if (owner.lower() == constants.REFUGEE):
            continue
        ...
        # alternate email
        alternateEmail = contact[constants.GANDHKOSH_FIELDS["Alternate E-mail"]]

        # alternate mobile
        alternateMobile = contact[constants.GANDHKOSH_FIELDS["Alternate Mobile"]]

        print(building, flatNo, firstName, secondName,
              email, mobile, alternateEmail, alternateMobile)

        # contacts are named on the building and flat number, and the owner name is added in organization field
        googleContact = {}
        googleContact[constants.GOOGLE_FIELDS["Name"]
                      ] = building + " " + flatNo
        ...
        googleContact[constants.GOOGLE_FIELDS["Group Membership"]
                      ] = constants.OWNER + " " + building + " ::: * myContacts"
        ...
        googleContact[constants.GOOGLE_FIELDS["Phone 2 - Value"]
                      ] = alternateMobile
        # this is a trick
        googleContact[constants.GOOGLE_FIELDS["Organization 1 - Name"]] = secondName
        # this is a trick
        googleContact[constants.GOOGLE_FIELDS["Organization 1 - Title"]] = firstName
...
def writeOutput(contactsData, fileName):
    googleContactFields = [constants.GOOGLE_FIELDS["Name"], constants.GOOGLE_FIELDS["Given Name"], constants.GOOGLE_FIELDS["Family Name"], GOOGLE_FIELDS["Notes"],
                           constants.GOOGLE_FIELDS["Group Membership"], constants.GOOGLE_FIELDS[
                               "E-mail 1 - Type"], constants.GOOGLE_FIELDS["E-mail 1 - Value"],
...
def processCommandline():
    parser = argparse.ArgumentParser(
        description='Convert to Google CSV format')
    parser.add_argument('--input', type=str, required=True,
                        help='input contacts CSV - Gandhakosh format')
    parser.add_argument('--output', type=str, required=True,
                        help='output contacts CSV - Google format')
